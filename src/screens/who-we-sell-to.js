...
function markup() {
  return `<header class="hero"><div class="brand-lockup"><div class="wordmark">LUMEN</div><span class="product-badge">Germany Launch Decision Tool</span></div><div><p class="eyebrow">Germany market entry</p><h1>Who do we sell to?<br><em>Choose the first audience.</em></h1><p class="lede">Compare German consumer segments, then focus the launch on the mix with the clearest acceptance and channel fit.</p></div></header><nav class="tabs" aria-label="Case workspaces"><button class="active" type="button" data-tab="who">Who do we sell to?</button><button type="button" data-tab="pricing">Pricing & unit economics</button><button type="button" data-tab="disagreements">Where the data disagrees</button></nav><details class="read-guide"><summary>How to read this screen</summary><p><b>Choose an audience:</b> select one or more segments to compare their tested price acceptance, preferred channel, and sample potential. <b>Use the recommendation panel:</b> it is a directional starting point rather than a sales forecast. <b>Read the launch strip:</b> Berlin and Munich are indicative opportunity sizes, not LUMEN revenue.</p></details><main class="audience-layout"><section class="segment-area"><div class="section-heading"><div><p class="eyebrow">Audience selection</p><h2>Start with the people most ready to try LUMEN</h2></div><p>Select one or more segments to update the recommendation.</p></div><div id="segment-cards" class="segment-cards" aria-label="Consumer segments"></div></section><aside id="recommendation" class="recommendation-panel" aria-live="polite"></aside></main><section id="launch-strip" class="launch-strip" aria-label="Launch location and timing"></section>`;
}
